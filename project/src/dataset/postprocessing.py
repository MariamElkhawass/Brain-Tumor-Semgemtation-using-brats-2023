...
def rm_tt_dust(pred_mat, tt):
    ...
    Args:
        pred_mat: Array of shape HWD for a predicted segmentation.
        tt: Either 'WT', 'TC' or 'ET' - the overlapping region for which to remove dust.
...
def fill_holes(pred_mat, tt, label, rm_dust_mask):
    ...
    Args:
        pred_mat: Array of shape HWD for a predicted segmentation.
        tt: Either 'WT', 'TC' or 'ET' - the overlapping region to detect holes in.
        label: The value that holes should be relabelled to.
        rm_dust_mask: The mask corresponding to the voxels removed as dust.
    """
    pred_mat_tt = get_tissue_wise_seg(pred_mat, tt)
...
def rm_dust_fh(pred_mat):
    ...
    Args:
        pred_mat: Array of shape (1, 4, H, W, D) for a predicted segmentation.

    Returns:
        The postprocessed array.
    """
    pred_mat_postprocessed = np.zeros_like(pred_mat)

    for i in range(pred_mat.shape[0]):
        pred_mat_slice = pred_mat[i, ...]
        rm_et_mask = rm_tt_dust(pred_mat_slice, 'ET')
        fill_holes(pred_mat_slice, 'TC', 1, rm_et_mask)

        rm_tc_mask = rm_tt_dust(pred_mat_slice, 'TC')
        fill_holes(pred_mat_slice, 'WT', 2, rm_tc_mask)

        _ = rm_tt_dust(pred_mat_slice, 'WT')

        rm_tc_mask = rm_tt_dust(pred_mat_slice, 'WT')

        pred_mat_postprocessed[i, ...] = pred_mat_slice

    return pred_mat_postprocessed




# Example usage:
# Assuming pred_mat is your predicted segmentation array
# postprocessed_array = rm_dust_fh(pred_mat)
