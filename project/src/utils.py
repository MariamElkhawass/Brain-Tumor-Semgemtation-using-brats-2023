...
def calculate_overall_accuracy(preds, targets):
    """
    Calculate the overall accuracy for each label separately.

    Parameters
    ----------
    preds : np.array
        Predictions tensor of size C*Z*Y*X.
    targets : np.array
        Ground truth tensor of same shape as preds.

    Returns
    -------
    accuracies : dict
        Dictionary containing the overall accuracy for each label.
    """
    ...
    for i, label in enumerate(labels):
        ...
        tp = np.sum(np.logical_and(preds[i], targets[i]))
        tn = np.sum(np.logical_and(np.logical_not(preds[i]), np.logical_not(targets[i])))

        # Calculate total number of pixels for the label
        total_pixels = np.sum(targets[i])

        # Calculate overall accuracy for the label
        if total_pixels > 0:
            accuracy = (tp + tn) / total_pixels
        else:
            accuracy = np.nan

        # Store the accuracy for the label
...
#         labelmap = np.zeros(segs.shape)  # Changed to match shape of segs
#         labelmap[0] = et
#         labelmap[1] = net
#         labelmap[2] = ed
...
#         # Remove dust and fill holes
#         pre_segs = rm_dust_fh(labelmap)  # Pass labelmap as argument

#         labelmap_sitk = sitk.GetImageFromArray(labelmap)  # Convert labelmap back to SimpleITK image
#         ref_seg_img = sitk.ReadImage(ref_path)
#         labelmap_sitk.CopyInformation(ref_seg_img)

#         print(f"Writing {args.seg_folder}/{patient_id}.nii.gz")
#         sitk.WriteImage(labelmap_sitk, f"{args.seg_folder}/{patient_id}.nii.gz")
...
def generate_segmentations(data_loader, model, writer, args):
    ...
    for i, batch in enumerate(data_loader):
        ...
        # Debug print
        print(f"pre_segs shape: {pre_segs.shape}")
        ...
        # Debug print
        print(f"segs shape before thresholding: {segs.shape}")

        segs = segs[0].numpy() > 0.5

        # Debug print
        print(f"segs shape after thresholding: {segs.shape}")

        # Ensure segs has the correct dimensions
        if segs.shape[0] != 3:
            raise ValueError(f"Unexpected segs shape: {segs.shape}, expected first dimension to be 3")
        ...
        # Postprocess the segmentation to remove dust and fill holes
        # Convert labelmap to 3D arrays for each class
        postprocessed_segs = np.zeros_like(labelmap)
        for i in range(labelmap.shape[0]):
            class_labelmap = labelmap[i]
            class_postprocessed = rm_dust_fh(class_labelmap[np.newaxis, ...])
            postprocessed_segs[i] = class_postprocessed

        # Convert to SimpleITK image
        postprocessed_segs_itk = sitk.GetImageFromArray(postprocessed_segs.astype(np.uint8))
        ref_seg_img = sitk.ReadImage(ref_path)
        postprocessed_segs_itk.CopyInformation(ref_seg_img)
        ...
        # Save segmentation
        print(f"Writing {args.seg_folder}/{patient_id}.nii.gz")
        sitk.WriteImage(postprocessed_segs_itk, f"{args.seg_folder}/{patient_id}.nii.gz")
